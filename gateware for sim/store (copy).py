...
class InputStore(SimpleElaboratable):
    ...
    def _elab_read(self, m, dps, r_full):
        ...
        m.d.comb += [
            ...
            input_width_count.restart.eq(self.restart),
        ...
        with m.Else():
                m.d.comb += mem_addrs[(r_addr[:2])].eq(mem_addrs[(r_addr[:2])])
                m.d.comb += mem_addrs[((r_addr+1)[:2])].eq( mem_addrs[((r_addr+1)[:2])])
                m.d.comb += mem_addrs[((r_addr+self.input_width+self.pad)[:2])].eq( mem_addrs[(r_addr+self.input_width+self.pad)[:2]])
                m.d.comb += mem_addrs[((r_addr+self.input_width+self.pad+1)[:2])].eq( mem_addrs[((r_addr+self.input_width+self.pad+1)[:2])])
        ...
        m.d.comb += tmp1.eq(Cat(smr_datas[r_addr[:2]], smr_datas[r_addr[:2]+1],
                           smr_datas[r_addr[:2]+2], smr_datas[r_addr[:2]+3]))
        m.d.comb += tmp2.eq(Cat(smr_mem_datas[r_addr[:2]], smr_mem_datas[r_addr[:2]+1],
                           smr_mem_datas[r_addr[:2]+2], smr_mem_datas[r_addr[:2]+3]))
        with m.If(self.switch_PorD == 0):
            tmp.eq(tmp1)
        with m.Else():
            tmp.eq(tmp2)
        ...
        a=Signal(12)
        b = [Signal(8) for n in range(4)]
        m.d.comb+=[b[0].eq(cut(tmp, 0, 8)),
        b[1].eq(-(cut(tmp, 8, 8).as_signed())),
        b[2].eq(-(cut(tmp, 4, 8).as_signed())),
        b[3].eq(cut(tmp, 12, 8))]
        m.d.sync +=a.eq((b[0]+b[1]+b[2]+b[3]).as_signed())
        c=Signal(8)
        d=Signal(8)
        m.d.sync +=c.eq(cut(tmp, 4, 8))
        m.d.sync +=d.eq(-(c.as_signed()))
